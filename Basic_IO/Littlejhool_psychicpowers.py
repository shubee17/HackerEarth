...
The algorithm which Little Jhool follows is, as following:
    ...
    If the binary number written by the person has six consecutive 0's, or 1's
...
"""
Binary_array = map(int, raw_input())
flag = 0
for i in range(len(Binary_array)):
    for j in range(i+1,len(Binary_array)+1):
        x = Binary_array[i:j]
        if len(x) == 6:
            if x.count(0) == 6 or x.count(1) == 6:
                print "Sorry, sorry!"
                flag = 1
                break
            else:
